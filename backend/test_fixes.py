...
r_pat = requests.post(f'{BASE}/api/patients', headers=H, json={
    'Patient_ID': 'TEST-DYNAMIC', 'Age': 55, 'Region': 'Northeast',
...
print('Score keys:', r_score.json().keys())
if 'risk_factors' in r_score.json():
    print('Factors:', [f['feature'] for f in r_score.json()['risk_factors']])
...
r_save = requests.post(f'{BASE}/api/patients/TEST-DYNAMIC/events', headers=H, json=req_save)
print('Save status:', r_save.status_code)
print('Save keys:', r_save.json().keys())
